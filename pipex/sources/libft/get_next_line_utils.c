...
/*   Updated: 2024/08/06 16:31:25 by yzheng           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*g_ft_strjoin(char *s1, char *s2)
{
	...
	str = (char *)malloc(((g_ft_strlen(s1)) + (g_ft_strlen(s2)) + 1));
	if (!str)
		return (ft_free(&s1));
	i = -1;
	j = 0;
	...
	while (s2[j] != '\0')
		str[i++] = s2[j++];
	str[g_ft_strlen(s1) + g_ft_strlen(s2)] = '\0';
	ft_free(&s1);
	return (str);
}
...
char	*getnewline(char *str)
{
	...
	if (!str[i])
		return (ft_free(&str));
	newstr = (char *)malloc((g_ft_strlen(str) - i + 1));
	if (!newstr)
		return (ft_free(&str));
	i++;
	j = 0;
